...
Chrome Extension học tiếng Anh từ bài đọc và video YouTube, trước đây là Mạch Đọc. Bản **0.6.0** thêm **Ôn 30 giây** ngay từ icon trên thanh công cụ (một thẻ đến hạn, tự gõ rồi tự chấm, dùng chung lịch FSRS với trang Ôn tập) và hai trò chơi trong Phòng Lab: **Ghép cặp** tính giờ và **Trắc nghiệm** với đáp án nhiễu lấy từ chính thư viện. Bản **0.5.2** sửa lỗi: bài tạo từ ghi chú không còn đề điền khuyết rỗng hay lộ đáp án, nghe chép chỉ tự bật với đoạn ngắn, chữ tiếng Việt trong ô trả lời không còn vỡ dấu, hàng đợi AI không gửi lại câu đã phân tích và xử lý nhanh hơn, lưu cài đặt giữ tham số FSRS đã hiệu chỉnh, **Sao lưu ngay** báo rõ khi lượt tải trước còn chạy. Bản **0.5.0** thêm **Phòng Lab**: lướt nhanh, bảng bong bóng tự lật và điền khuyết; dùng thư viện có sẵn, không gọi AI, giữ nguyên lịch FSRS. Vẫn có nhiều nhà cung cấp AI dự phòng, thư viện thẻ gọn, nhóm/nhãn màu, tìm kiếm không dấu và phân trang. Font Noto Sans tiếng Việt được đóng gói cùng extension; nhà phát hành **Blue**. TypeScript strict, MV3, React, IndexedDB, FSRS. Không có backend riêng; AI, từ điển và video gốc cần mạng.

**Ôn 30 giây:** bấm icon LumaRead trên thanh công cụ → gõ câu trả lời → **Đối chiếu** (`Ctrl+Enter`) → chấm `1`–`4`. Không gửi AI; muốn AI chấm hoặc nghe chép thì mở trang Ôn tập. Trên tab YouTube, popup có thêm nút **Lưu câu vừa nghe** (phím tắt `Alt+Shift+Y` vẫn lưu ngay).

**Phòng Lab:** chọn ở thanh điều hướng → chọn chế độ → lọc nguồn/nhóm/nhãn/tiến độ → đặt số mục và thời gian → bắt đầu. Bản **0.6.0** thêm **Ghép cặp** (bảng 4/6/8 cặp, đồng hồ, ghép nhầm được đánh dấu) và **Trắc nghiệm** (Anh → Việt, Việt → Anh hoặc trộn; phím `1`–`4`). Bản **0.5.1** thêm **Lặp lại liên tục** cho Lướt nhanh/Bong bóng: hết bộ từ tự chạy lại cho đến khi bấm **■ Dừng**, kể cả chỉ có một từ. Nhập số mục **0** để luyện tất cả; bấm thẻ bong bóng để dừng và xem đầy đủ. Có toàn màn hình, ẩn nghĩa, đánh dấu luyện lại và tự tạm dừng khi chuyển tab. Xem [hướng dẫn Lab và thiết kế](docs/LAB.vi.md).
...
3. Mở tab mới hoặc `Alt+Shift+R`. Vào **Cài đặt & dữ liệu** → **Nhà cung cấp AI & dự phòng** để nhập khóa Gemini, DeepSeek, OpenAI hoặc GLM nếu muốn dùng AI. Icon trên thanh công cụ mở popup **Ôn 30 giây**; trên YouTube popup có nút **Lưu câu vừa nghe**.
...
- **Phân tích sau:** trong Thư viện chọn các đoạn → **Phân tích (N)** (chỉ tính các đoạn chưa có phân tích; đoạn đã phân tích chờ bạn **Duyệt**). Xử lý tuần tự từng đoạn; mỗi phút một lượt, mỗi lượt tối đa 3 đoạn và dừng ngay khi gặp lỗi. Chưa nhập key thì hàng đợi giữ nguyên. Rate limit/lỗi mạng được retry có backoff, tối đa 4 lần. Job đang chạy bị gián đoạn được nhận lại sau khi lease 90 giây hết.
...
- **Không dùng AI:** ghi nghĩa cho một đoạn ngắn dưới 500 ký tự rồi chọn **Tạo bài từ ghi chú**. Bài thủ công không có ví dụ mới do AI sinh; khi ôn luôn là dạng tự viết lại từ nghĩa (gợi ý chỉ hiện chữ cái đầu), không có câu điền khuyết rỗng hay đề lộ đáp án.
...
- **Nghe chép chính tả:** tự xen vào lượt ôn thứ tư khi đoạn nghe không quá 40 từ, hoặc chọn **Đổi sang nghe chép chính tả** (tối đa 300 từ). Câu web quá dài được thay bằng trích dẫn ngắn của bài học; clip YouTube luôn dùng đúng câu trong clip. Gõ trước khi đối chiếu; chấm từng từ offline, lưu lỗi vào hồ sơ. Không hiện panel note trong player ôn tập. Video có chữ ghi sẵn trong hình vẫn có thể lộ lời nói.
...
`npm test` có 169 kiểm thử dữ liệu, phụ đề/timing, lỗi/quota, nâng cấp DB v1/v2→v3, nhóm/nhãn, Phòng Lab, hồi quy lỗi và kiểm định tham số. `npm run test:e2e` chạy bảy bộ: luồng đọc, YouTube/hồ sơ, tương thích Gemini, thư viện 5.000 câu và font tiếng Việt, nhiều nhà cung cấp/dự phòng, Phòng Lab và popup Ôn 30 giây. `npm run check:text` kiểm tra UTF-8, manifest và kích thước icon. YouTube được mô phỏng bằng native media/TextTrack; AI và từ điển có response giả lập; E2E dự phòng mô phỏng xác nhận quyền host của Chrome headless. `node scripts/performance.mjs` kiểm tra riêng 3.000 unit đánh dấu trên trang dài với CPU throttle. Kết quả/screenshot nằm trong `test-results/`; không dùng key thật.
