# Nghiệm thu bản 0.6.0 — LumaRead

Kiểm tra ngày **21/09/2026** trên Windows, Node.js **22.13.1**, Chromium **149.0.7827.55**, hồ sơ thử riêng: TypeScript strict, build, kiểm tra UTF-8 **70 file** và **169/169 unit tests** đạt. Có 10 kiểm thử mới trong `tests/lab-games.test.ts`: câu trắc nghiệm luôn có đúng một đáp án đúng và không có đáp án nhiễu hợp lệ, ưu tiên cùng loại, chiều Việt → Anh, thư viện một mục; chia bảng ghép cặp dùng mỗi mục đúng một lần, đúng kích thước, không để hai ô trùng chữ; lọc mục ngắn; thiết lập mới tương thích dữ liệu cũ; hàng đợi `reviewQueue` và cách hiện khoảng ôn `dueIn` dùng chung cho trang Ôn tập và popup.

Bảy bộ E2E chạy trên dist **0.6.0** và đều đạt: [capture/ôn tập](../test-results/e2e-report.json) **14**, [YouTube/học bổ sung](../test-results/enrichment-e2e-report.json) **26**, [Gemini](../test-results/gemini-e2e-report.json) **5**, [thư viện](../test-results/organization-e2e-report.json) **10**, [AI dự phòng](../test-results/ai-fallback-e2e-report.json) **11**, [Lab](../test-results/lab-e2e-report.json) **20** (18 cũ + 2 mới), [popup Ôn 30 giây](../test-results/popup-e2e-report.json) **7** (bộ mới). Tổng **93 kiểm tra**. Thư viện 5.000 câu, CPU chậm 4×: tải 1.761 ms, tìm 409 ms, mở chi tiết 310 ms (gồm thao tác Playwright).

**Popup Ôn 30 giây** ([báo cáo](../test-results/popup-e2e-report.json)): manifest mở `popup.html` từ icon; popup hiện đúng mục kế tiếp và số mục đến hạn như trang Ôn tập; nút **Đối chiếu** chờ có câu trả lời, `Ctrl+Enter` mở đáp án, `Enter` sau đó không chấm nhầm; phím `3` ghi `Review` (rating 3, không assisted, câu trả lời đã gõ), `reps` tăng, lịch dời xa hơn 1 giờ, badge giảm theo; **Chưa nhớ · Xem đáp án** chỉ cho chấm `1` và ghi `assisted`; **Bỏ qua** không đổi lịch/lịch sử; hết hàng đợi hiện trạng thái rỗng; khổ 380 px không tràn ngang; không lỗi trang. Chrome headless không bấm được icon thanh công cụ nên popup được mở như một trang extension; nút **Lưu câu vừa nghe** chỉ hiện trên tab YouTube và gửi đúng thông điệp `capture-video` mà phím `Alt+Shift+Y` vẫn dùng (bộ YouTube/học bổ sung kiểm tra luồng lưu này).

**Phòng Lab** ([báo cáo](../test-results/lab-e2e-report.json)) thêm 2 kiểm tra: Ghép cặp (không cho bắt đầu với 1 mục; bảng 4 cặp đủ 8 ô; ghép nhầm rung đỏ và không tính tiến độ; tạm dừng khóa bảng; màn hình 390 px không tràn; tổng kết "4 / 4 · 2 cặp ghép đúng ngay · 1 lần ghép nhầm", đánh dấu 2 mục nhầm) và Trắc nghiệm (4 đáp án khác nhau lấy từ thư viện; chọn bằng phím; phản hồi đúng/sai; đánh dấu khi sai; tổng kết "2 câu chọn đúng · 1 mục để xem lại"; chiều Việt → Anh ẩn nút nghe tới khi trả lời). Snapshot cuối bộ Lab xác nhận captures, lịch FSRS, reviews, assessments và usage không đổi; không có cuộc gọi mạng/AI.

**Giới hạn:** chưa kiểm thử bấm icon thật trên Chrome có giao diện; popup nặng ~680 KB (phần lớn là `zod` dùng chung) và mở trong khoảng 240–280 ms trên máy kiểm thử (gồm thao tác Playwright). Ghép cặp và Trắc nghiệm không cập nhật FSRS, giống các chế độ Lab khác.

## Lịch sử nghiệm thu 0.5.2
