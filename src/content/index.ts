...
host.style.cssText = 'all:initial!important;position:fixed!important;inset:auto!important;margin:0!important;padding:0!important;border:0!important;background:transparent!important;overflow:visible!important;';
...
function hide() { if (host.matches(':popover-open')) host.hidePopover(); }
...
function openEditor(source: Source) {
  ...
  dialog.addEventListener('close', () => { host.style.removeProperty('display'); icon.hidden = false; previousFocus?.focus({ preventScroll: true }); });
...
async function refreshHighlights() {
  ...
  try { const patterns = await send<Pattern[]>({ type: 'lexicon' }); if (version === refreshVersion && patterns.length) stopHighlights = startHighlights(patterns); }
