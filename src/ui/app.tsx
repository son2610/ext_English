...
import { allData, settings, saveGeneralSettings, enqueue, acceptAnalysis, addManualUnit, findDuplicate, recordReview, reviseUnit } from '../data/repository';
...
import { getProvider as provider } from '../ai/factory';
...
import { gradeDictation, alignDictation, dictationPassage, tokens, DICTATION_AUTO_WORDS, DICTATION_MAX_WORDS } from '../learning/errors';
...
function App() {
  ...
  // Recomputed when data is reloaded, not on every notice/busy re-render; review history grows for years.
  const { queue, stats, difficult } = useMemo(() => ({
    queue: reviewQueue(data.units, data.reviews, config, ranks),
    stats: statistics(data.units, data.reviews),
    difficult: data.units.filter(u => u.leech || u.suspended),
  }), [data, config, ranks]);
  return <div className="shell">
    ...
    <main>
      ...
      {message && !libraryOverlay && <div className="notice" role="status"><span>{message}</span><button aria-label="Đóng thông báo" onClick={() => setMessage('')}>×</button></div>}
      ...
      {!loaded ? <div className="loading">Đang mở góc học của bạn…</div> : <div className="page">
        ...
        <footer>LUMAREAD {appVersion} <span>·</span> Từ điều bạn đọc, thành điều bạn biết.<br/>Blue <span>·</span> <a href="privacy.html" target="_blank" rel="noopener noreferrer">Quyền riêng tư</a> <span>·</span> <a href="mailto:phamhongson5151@gmail.com">Hỗ trợ</a></footer>
      </div>}
    </main>
  </div>;
}
...
function CaptureCard({ capture: c, units, busy, run }: { capture: Capture; units: Unit[]; busy: boolean; run: Run }) {
...
function ReviewPanel({ queue, captures, config, refresh, notify }: { queue: Unit[]; captures: Capture[]; config: Settings; refresh: () => Promise<void>; notify: (text: string) => void }) {
  ...
  const dictationText = dictationPassage(source, unit?.knowledge.evidence ?? '');
  const dictationWords = tokens(dictationText).length;
  // Long passages are never chosen automatically; beyond the aligner's limit dictation is not offered at all.
  const dictationAvailable = dictationWords > 0 && dictationWords <= DICTATION_MAX_WORDS;
  const dictation = dictationAvailable && (dictationChoice ?? (!!unit && unit.schedule.reps % 4 === 3 && dictationWords <= DICTATION_AUTO_WORDS));
