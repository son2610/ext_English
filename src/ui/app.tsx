...
function App() {
  ...
  const { queue, stats, difficult } = useMemo(() => {
    ...
    const startedToday = data.reviews.filter(r => r.prior.reps === 0 && dayKey(r.at) === today).length;
    return {
      queue: dueQueue(data.units, Date.now(), Math.max(0, config.dailyNewLimit - startedToday), unit => learningPriority(unit, ranks, config.learnerLevel)),
