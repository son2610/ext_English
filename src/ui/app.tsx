...
type Page = 'home' | 'review' | 'library' | 'difficult' | 'settings';
type Data = Awaited<ReturnType<typeof allData>>;
const empty: Data = { captures: [], units: [], reviews: [] };
const pageNames: Record<Page, string> = { home: 'Góc học hôm nay', review: 'Ôn tập', library: 'Thư viện ngữ cảnh', difficult: 'Cần chăm sóc', settings: 'Cài đặt & dữ liệu' };
...
async function provider(): Promise<AIProvider> {
  ...
  return new GeminiProvider(typeof key.geminiKey === 'string' ? key.geminiKey : '', config.model);
}
...
function App() {
  ...
  const queue = dueQueue(data.units, Date.now(), Math.max(0, config.dailyNewLimit - startedToday));
...
function Library({ data, busy, run }: { data: Data; busy: boolean; run: Run }) {
  ...
  const filtered = data.captures.filter(c => (!search || normalize(`${c.source.exact} ${c.note} ${c.analysis?.meaningVi ?? ''}`).includes(normalize(search))) && (filter === 'all' || (filter === 'pending' ? !c.unitsCreated : c.unitsCreated))).sort((a, b) => b.source.capturedAt - a.source.capturedAt);
...
function ReviewPanel({ queue, captures, config, refresh, notify }: { queue: Unit[]; captures: Capture[]; config: Settings; refresh: () => Promise<void>; notify: (text: string) => void }) {
  ...
  const task = unit ? exercise(unit) : undefined;
  const reset = () => { setAnswer(''); setRevealed(false); setAssisted(false); setGrade(undefined); setStarted(Date.now()); setReviewId(crypto.randomUUID()); };
  async function checkAI() {
    if (!unit || !task || !answer.trim()) return;
    setBusy(true);
    try { const ai = await provider(); setGrade(await ai.grade(unit.knowledge, task.prompt, answer)); setRevealed(true); }
