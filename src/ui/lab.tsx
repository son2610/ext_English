...
import { buildLabCards, checkLabAnswer, defaultLabPreferences, filterLabCards, LabPreferencesSchema, sampleLabCards, type LabCard, type LabMode, type LabPreferences } from '../domain/lab';
...
export function Lab({ captures, units, config, onActiveChange }: { captures: Capture[]; units: Unit[]; config: Settings; onActiveChange: (active: boolean) => void }) {
  ...
  if (session) return <LabSession key={session.id} cards={session.cards} preferences={session.preferences} onExit={() => setSession(undefined)} onReplay={deck => setSession(s => s ? { ...s, cards: deck, id: Math.max(Date.now(), s.id + 1) } : s)}/>;
  return <div className="lab-workspace">
    ...
    <section className="lab-setup panel"><div className="lab-section-title"><h2>02 <span>Chỉnh theo nhịp của bạn</span></h2><span className="lab-match-count">{eligible.length.toLocaleString('vi-VN')} mục phù hợp</span></div>
      ...
      {preferences.mode !== 'cloze' && <label className="checkbox-label lab-recall-option"><input aria-label="Lặp lại liên tục" type="checkbox" checked={preferences.repeat} onChange={e => change('repeat', e.target.checked)}/>Lặp lại liên tục<small>Hết bộ từ sẽ chạy lại. Bấm nút ■ Dừng để kết thúc.</small></label>}
      <div className="lab-launch"><div><strong>{currentMode.name} · {count.toLocaleString('vi-VN')} mục{preferences.repeat && preferences.mode !== 'cloze' ? ' / vòng · ∞' : ''}</strong><p>{preferences.mode === 'cloze' ? 'Tự gõ, đối chiếu đáp án rồi chuyển câu. Không giới hạn thời gian.' : `Khoảng ${durationText(labTimeline(preferences.mode, 0, count, preferences.seconds, preferences.columns ** 2).durationMs)}${preferences.repeat ? ' mỗi vòng · Tự lặp cho đến khi bạn bấm Dừng.' : ' · Mỗi mục xuất hiện một lần trong lượt.'}`}</p></div><button className="primary lab-start" disabled={busy || !count} onClick={() => void start()}>Bắt đầu lượt luyện <span aria-hidden="true">→</span></button></div>
      {error && <p className="error-text" role="alert">{error}</p>}
      {!eligible.length && <div className="lab-empty" role="status"><strong>Chưa có mục phù hợp để bắt đầu.</strong><p>{cards.length ? 'Thử chọn tất cả nguồn, nhóm, nhãn hoặc tiến độ. Điền khuyết cần bài học đã có câu và đáp án hợp lệ.' : 'Thêm nghĩa hoặc ghi chú cho câu đã lưu, hoặc duyệt phân tích để tạo bài học. Lab chỉ dùng nội dung bạn đã có.'}</p><a href="#library">Mở thư viện ngữ cảnh ↗</a></div>}
    </section>
...
function LabSession({ cards, preferences, onExit, onReplay }: { cards: LabCard[]; preferences: LabPreferences; onExit: () => void; onReplay: (cards: LabCard[]) => void }) {
  ...
  const clock = useLabClock(preferences.mode === 'cloze' ? 1000 : 50); const [ended, setEnded] = useState(false); const [focus, setFocus] = useState<LabCard>();
  ...
  const timed = preferences.mode !== 'cloze';
  ...
  const done = ended || (timed ? frame.done : index >= cards.length);
  ...
  function speak(card: LabCard) {
    pause();
    if (!('speechSynthesis' in window)) { setNotice('Trình duyệt chưa hỗ trợ đọc thành tiếng.'); return; }
    speechSynthesis.cancel(); const speech = new SpeechSynthesisUtterance(card.english); speech.lang = 'en-US'; speech.rate = 0.9;
