...
export function Library({ captures, units, busy, run, notice, renderDetail, onOverlayChange }: {
  ...
  return <section className="library-workspace">
    ...
    {!!selected.size && <div className="bulk-bar" aria-label="Thao tác hàng loạt"><strong>{selected.size} câu đã chọn</strong>
      ...
      <button disabled={busy || !actionable.length} onClick={() => void run(async () => { await enqueue(actionable.map(c => c.id)); await send({ type: 'wake' }); setSelected(new Set()); }, 'Đã xếp hàng phân tích. AI sẽ nhận đoạn trích, ngữ cảnh và ghi chú qua các nhà cung cấp AI đã cấu hình.')}>Phân tích ({actionable.length})</button>
      <button aria-label="Bỏ chọn tất cả" onClick={() => setSelected(new Set())}>Bỏ chọn</button>
      <small>Phân tích AI gửi câu, ngữ cảnh và ghi chú tới các nhà cung cấp đã cấu hình, kể cả dự phòng nếu bật. Hàng đợi xử lý tuần tự.</small>
    </div>}
